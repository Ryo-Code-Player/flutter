<?php
    $new_pros = \App\Models\Product::where('status','active')->where('is_sold',1)->orderBy('id','desc')->limit(6)->get();
    $banners = \DB::select("SELECT * from banners where   status = 'active' and `condition`='modpro'  order by rand() LIMIT 1");;
    if(count($banners) > 0)
        $banner = $banners[0];
?>
...
<section class="wrapper !bg-[#ffffff]">
    ...
    <div class="container pb-[1.5rem] xl:pb-10 lg:pb-10 md:pb-10 pt-14">
        <div class="post-header mt-[-50px]">
            <div class="">
            <h2><a href="<?php echo e(route('front.product.hot')); ?>" class=" hover" rel="category"><i class="uil uil-box"></i> SẢN PHẨM MỚI</a></h2>
            </div>
            <!-- /.post-category -->
        </div>
        <div class="flex flex-wrap mx-[-15px] mt-[-0px]">
            ...
            <!--/.row -->
                <div class="itemgrid grid-view projects-masonry shop mb-5">
                    <div class="flex flex-wrap mx-[-15px] xl:mx-[-0px] lg:mx-[-0px] md:mx-[-0px] mt-[-0px] xl:mt-[-0px] lg:mt-[-0px] md:mt-[-0px] isotope">
                        <?php $__currentLoopData = $new_pros; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pro): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                            $photos = explode( ',', $pro->photo);
                        ?>
                            <div class="project item group md:w-6/12 lg:w-4/12 xl:w-4/12 w-full flex-[0_0_auto] xl:px-[10px] lg:px-[10px] md:px-[10px] px-[10px] mt-[10px] xl:mt-[10px] lg:mt-[10px] md:mt-[10px] max-w-full">
                                ...
                                <div class="post-header">
                                    ...
                                    <h3  style="height:60px; overflow:hidden"  class="   text-[1.1rem]"><a href="<?php echo e(route('front.product.view',$pro->slug)); ?>" 
                                        class="title_color"><?php echo e($pro->title); ?></a>
                                    </h3>
                                    <p class="price !m-0">  <ins class="no-underline text-[#e2626b]">
                                        <span class="amount"><?php echo e(number_format($pro->price,0,'.',',')); ?> đ</span></ins>
                                    </p>
                                </div>
                            </div>
                             <!-- /.item -->
      ...
      <!-- /.container -->
</section><?php /**PATH E:\Flutter\codemoi\flutterbekeryapp 2\flutterbekeryapp 2\backend\resources\views/frontend_tp/layouts/mod6pro.blade.php ENDPATH**/ ?>